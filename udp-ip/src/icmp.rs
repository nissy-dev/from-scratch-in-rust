...
impl Icmp {
    pub fn send(
        ...
        let icmp_data = [0x01, 0x02];
        let packet_length =
            (ETHERNET_FRAME_LENGTH + IP_HEADER_LENGTH + ICMP_HEADER_LENGTH + icmp_data.len())
                as u16;
        let ethernet_frame =
        ...
        let icmp_frame = IcmpFrame::new(MessageType::Echo, &icmp_data);
        let send_packet = [
        ...
        while let Ok((_ret, _addr)) = reciever.recvfrom() {
            if !reciever.buf.is_empty() && Icmp::is_icmp_reply_packet(&reciever.buf) {
                ...
                let offset = ETHERNET_FRAME_LENGTH + ICMP_HEADER_LENGTH;
...
impl IcmpFrame {
    fn new(frame_type: MessageType, data: &[u8]) -> IcmpFrame {
        ...
        IcmpFrame {
            code: 0,            // 常に 0
            checksum: 0,        // 後でセットする
            identification: 16, // なんでも良いので、今回は 0
            seq_num: 1,         // 初期値はなんでも良いので、今回は 0
            frame_type,
            data: data.to_vec(),
        }
    }
